...
def task_5_delete_the_last_customer(con) -> None:
    ...
    """
    with con.cursor() as cursor:
        #k=SELECT COUNT(CustomerID) FROM Customers
        cursor.execute("""DELETE FROM Customers 
                          WHERE CustomerID = (SELECT COUNT(CustomerID) FROM Customers )""")
...
def task_8_count_customers_by_city(cur):
    ...
    """
    cur.execute("""SELECT City, COUNT(City) FROM Customers 
                GROUP BY City 
                ORDER BY City DESC""")
    return cur.fetchall()
...
def task_9_count_customers_by_country_with_than_10_customers(cur):
    ...
    Returns: 3 recordsCountry
    """
    cur.execute("""SELECT Country, COUNT(Country) FROM Customers 
...
def task_13_list_products_from_sweden_suppliers(cur):
    ...
    cur.execute("""SELECT p.ProductName FROM Products AS p
                   ...
                   ON p.SupplierID = s.SupplierID WHERE s.Country = 'Sweden'""")
    return cur.fetchall()
...
def task_16_match_all_customers_and_suppliers_by_country(cur):
    ...
    cur.execute("""SELECT c.CustomerName, c.Address, c.Country AS CustomerCountry,
                   ...
                   ON c.Country = s.Country ORDER BY c.Country, s.Country"""
    )
    return cur.fetchall()